/*
Esta base de datos está diseñada para gestionar todo el relacionado con artistas musicales y su actividad, 
combinando información artística con comercio online. Incluye tablas para almacenar datos de artistas 
(como nombre, fecha de nacimiento o redes sociales), sus álbumes y canciones (con detalles como duración o enlaces), 
además de productos oficiales asociados (como álbumes, vinilos o merchandising). 

También gestiona eventos como conciertos o festivales, y un sistema completo de compra con carrito, 
pedidos y sus respectivos productos, incluyendo estados del pedido. 
Por último, incorpora una tabla de usuarios para el acceso al sistema. 
Todo está estructurado con claves primarias, foráneas y múltiples restricciones (NOT NULL, UNIQUE, CHECK) 
...
CONSTRAINT fk_orders_user_id FOREIGN KEY (user_id) REFERENCES users (users_id) ON DELETE CASCADE,
