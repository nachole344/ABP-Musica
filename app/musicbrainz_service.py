...
def search_albums(artist_name=None, mb_artist=None, album_name=None, mb_release=None):
    if not artist_name and not mb_artist and not mb_release and not album_name:
        raise ValueError("Proporciona artist_name, mb_artist, mb_release o album_name")

    resolved_artist_id = mb_artist
    if not resolved_artist_id and not mb_release and artist_name:
    ...
    if mb_release:
        ...
        if resolved_artist_id:
            q += f"%20AND%20arid:{resolved_artist_id}"
        ...
        full = mb_get(f"/release/{match['id']}?inc=recordings+url-rels&fmt=json")
        releases = [full]
    else:
        data = mb_get(
    ...
    results = []
    for rel in releases:
        full = rel if rel.get("media") else mb_get(f"/release/{rel['id']}?inc=recordings+url-rels&fmt=json")
